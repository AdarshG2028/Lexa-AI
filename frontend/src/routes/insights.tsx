      ...
      {grammar && (
        <section className="relative mx-auto w-full max-w-5xl px-6 pb-16">
          <div className="flex items-center gap-3">
            <SpellCheck className="size-5 text-accent" />
          ...
          {grammar.issues.length === 0 ? (
            ...
            <div className="mt-6 space-y-4">
              {grammar.issues.map((issue) => (
                <article key={issue.id} className="surface rounded-2xl p-6">
      ...
      <section className="relative mx-auto w-full max-w-5xl px-6 pb-24">
        {PRONUNCIATION_ENABLED && (
          <>
            ...
            {!pronunciation && (
              <div className="surface mt-6 rounded-2xl p-8">
                ...
                <button
                  ...
                  className="mt-6 inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-3 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-60"
                >
                  {pronRunning ? (
                    <>
                ...
                {pronunciation.issues.length > 0 && (
                  <div className="mt-6 grid gap-4 md:grid-cols-3">
                    {pronunciation.issues.map((issue) => (
                      <article key={issue.id} className="surface flex flex-col rounded-2xl p-6">
        ...
        <div className="surface glow mt-10 flex flex-col items-center gap-4 rounded-2xl p-10 text-center">
          ...
          <Link
            to="/conversation"
            className="rounded-lg bg-primary px-6 py-3 text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-0.5"
          >
...
function Stat({ label, value, caption }: { label: string; value: number | null; caption: string }) {
  return (
    <div className="surface rounded-2xl p-6">
      ...
      <p className="mt-2 font-mono text-4xl tabular-nums text-foreground">{value ?? "—"}</p>
